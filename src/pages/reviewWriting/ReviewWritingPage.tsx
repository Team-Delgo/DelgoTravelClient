  ...
  const handleUploadFile = async (event: { target: HTMLInputElement }) => {
    ...
    const showCroppedImage = async () => {
      try {
        ...
        setSendingImage([...sendingImage, newFile]);
        setImg('')
      } catch (e) {
        console.error(e);
      }
          ...
          (response: AxiosResponse) => {
            ...
            if (code === 200) {
              if (sendingImage.length > 0) {
                ...
                reviewImageUpload(formData, reviewId, dispatch, (response: AxiosResponse) => {
    ...
    return (
      <div className="review-writing">
        ...
        {reviewCompleteAlert && (
          <AlertConfirmOne text="리뷰 작성이 완료 되었습니다" buttonHandler={confirmReviewCompletedClose} />
        )}
        {reviewTextLengthLimitAlert && (
          <AlertConfirmOne text="리뷰를 최소 10글자이상 작성하세요" buttonHandler={alertReviewLengthLimitClose} />
        )}
