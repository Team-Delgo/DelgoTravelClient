...
import { years, months, days } from './Birth';
...
function BirthSelector(props: { changeBirth: (year:number,month:number,day:number)=>void }) {
  ...
  const yearContext = years.map((year,i) => {
    ...
    return <div key={year} className={classNames('birth-item',{selected:year===selectedYear})}>{`${year}년`}</div>;
  });
  const monthContext = months.map((month,i) => {
    if(month==='.') return <div className='birth-item blank'>.</div>
    return <div key={`m${month}`} className={classNames('birth-item',{selected:month===selectedMonth})}>{`${month}월`}</div>;
  });
  const dayContext = days.map((day,i) => {
    if(day==='.') return <div className='birth-item blank'>.</div>
    return <div key={`d${day}`} className={classNames('birth-item',{selected:day===selectedDay})}>{`${day}일`}</div>;
  });
